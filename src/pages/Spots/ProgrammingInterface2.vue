        ...
        <div class="form-group mt-4 p-4 bg-dark-secondary rounded-lg border border-dark-border">
          <div class="flex items-center justify-between mb-3">
            <label class="label mb-0">
              ...
              Selecccionar hora y minuto de salidas
            </label>
            <button
            ...
            >
              ...
              <div class="grid grid-cols-6 sm:grid-cols-10 gap-2">
                ...
                >
                  ...
                  <i
v-if="isMinuteProgrammed(minute)"
class="fa fa-ban text-xs"
/>
                  <i
v-else-if="isMinuteSelected(minute)"
...
>
          ...
          <!-- Resumen de salidas pautadas pendientes -->
          ...
          <div>
            <h4 class="flex items-center gap-2 text-sm font-semibold text-text-primary mb-3">
              ...
              Todas las salidas pautadas ({{ pendingProgramaciones.length }})
            </h4>
            <div
class="max-h-64 overflow-y-auto space-y-2"
@scroll="handlePendingScroll"
>
              <div
...
>
              ...
              <div
v-else-if="pendingProgramaciones.length > 0"
class="flex items-center justify-center gap-2 py-3 text-success-400"
>
                <i class="fa fa-check-circle" />
                <span class="text-sm">Has visto todas las {{ pendingProgramaciones.length }} salidas pautadas pendientes</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <template #footer>
        ...
        >
          <i class="fa fa-check" />
          Confirmar y Guardar ({{ pendingProgramaciones.length }} salidas pautadas)
        </button>
      </template>
    </Modal>
...
>
      ...
      <div class="p-4 bg-dark-secondary rounded-lg border border-dark-border mb-4">
        <div class="form-group">
          ...
          <small
v-if="filterStartTime && filterEndTime"
class="text-xs text-info-400 mt-2 flex items-center gap-1"
>
            <i class="fa fa-info-circle" />
            Mostrando salidas entre {{ filterStartTime }} y {{ filterEndTime }}
          </small>
        </div>
      </div>

      <!-- Mensaje cuando no hay salidas pautadas -->
      <div
v-if="filteredProgramaciones.length === 0"
        ...
        <div class="space-y-4">
          <div class="alert alert-info">
            ...
            <p>
              ...
              pauta(s) en el servidor.
            </p>
          </div>

          <div class="p-4 bg-dark-secondary rounded-lg space-y-2">
            <div class="flex items-center justify-between text-sm">
              <span class="text-text-secondary">Total de pautas:</span>
            ...
            <div class="flex items-center justify-between text-sm">
              <span class="text-text-secondary">Spots involucrados:</span>
...
>
        ...
        <button
class="btn btn-primary"
@click="confirmAndSaveProgramaciones"
>
          <i class="fa fa-check" />
          Confirmar y Guardar
        </button>
...
Streaming
            ...
            <div
v-if="hasSpotModalFilters"
class="mt-3 flex justify-end"
>
              <button
class="btn btn-ghost btn-sm"
@click="clearSpotModalFilters"
>
                <i class="fa fa-times" />
                Limpiar filtros
              </button>
...
>
          ...
          <div class="max-h-[50vh] overflow-y-auto pr-2">
            <div
v-if="filteredSpotsForModal.length === 0"
class="text-center py-12"
>
...
>
      ...
      <template #footer>
        <div class="flex items-center justify-between w-full">
          <div class="text-sm text-text-secondary">
            <span
v-if="!canAddMoreSpots && selectedSpots.length >= maxSpotsAllowed"
...
>
          ...
          <div class="flex gap-2">
            <button
class="btn btn-ghost"
@click="showSpotSelectorModal = false"
>
              Cancelar
            </button>
            <button
...
Guardando salidas programadas...
            ...
            <div class="mt-4 flex items-center gap-2 text-sm text-text-secondary">
              <i class="fa fa-info-circle" />
              <p>Guardando salidas pautadas... Por favor espera.</p>
            </div>
          </div>
        </div>
      </transition>
